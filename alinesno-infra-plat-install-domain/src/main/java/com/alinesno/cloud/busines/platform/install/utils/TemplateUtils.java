...
public class TemplateUtils {
	...
	static {
		String k8sTemplatePath = NetUtils.getInstallFile() + File.separator + Const.DIR_K8S ; 
		k8sEngine = new VelocityEngine();
		k8sEngine.setProperty(Velocity.FILE_RESOURCE_LOADER_PATH, k8sTemplatePath);
		k8sEngine.setProperty(Velocity.RUNTIME_LOG, NetUtils.getInstallFile() + File.separator + "velocity-"+ Const.DIR_K8S +".log");
		k8sEngine.setProperty(Velocity.INPUT_ENCODING, "UTF-8");
		k8sEngine.setProperty(Velocity.OUTPUT_ENCODING, "UTF-8");
		k8sEngine.init();
		
		String dockerTemplatePath = NetUtils.getInstallFile() + File.separator + Const.DIR_DOCKER ; 
		dockerEngine = new VelocityEngine();
		dockerEngine.setProperty(Velocity.RUNTIME_LOG, NetUtils.getInstallFile() + File.separator + "velocity-" + Const.DIR_DOCKER +  ".log");
	...
	public static void writeK8STemplate(AIPYaml yaml, Map<String, String> config) throws IOException {
		...
		String writePath = NetUtils.getInstallFile() + File.separator + Const.DIR_K8S + File.separator  +  yaml.getFileName();
	...
	public static void writeDockerTemplate(AIPYaml yaml, Map<String, String> config) throws IOException {
		...
		String writePath = NetUtils.getInstallFile() + File.separator + Const.DIR_DOCKER + File.separator  +  yaml.getFileName();
